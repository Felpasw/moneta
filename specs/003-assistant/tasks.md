...
- [x] **MNT-250** [T][S] ✅ commit `8b14648` — Categorizar vozes por match com `outputLanguage` do profile + aviso na UI — hoje `GET /agent/voices` (MNT-56) devolve todas as vozes do ElevenLabs em lista flat, com `labels.language` cru (string tipo `"en"`, `"pt"`), sem normalização. Front (`web/src/components/organisms/AssistantSettingsVoice.tsx`) mostra grid único sem nenhuma pista de que voz X vai soar estranha no idioma que o user configurou (MNT-217). Nota do MNT-217 já registra que "voz EN falando PT-BR funciona mas com sotaque estranho" — o nudge existe no playbook do LLM mas nada avisa o **user na UI**. Fix, backend-driven (memória: front não faz reshape/filtro de shape):
  - Backend: estender port `TtsVoice` com `languageMatch: 'match' | 'mismatch' | 'unknown'` computado no use-case a partir do `outputLanguage` do profile do caller. `match` = idioma normalizado da voz bate com `outputLanguage`; `mismatch` = normaliza pra idioma diferente (soa esquisito); `unknown` = label ausente/lixo (a gente não sabe garantir). Decisão consciente de NÃO ter bucket `multilingual` — ElevenLabs não expõe isso no `labels.language`; pra saber real precisaria ler `high_quality_base_model_ids` e é overengineering pra 1ª iteração.
  - Normalizador puro `mapElevenLabsLanguage(raw?: string): 'pt_BR' | 'en_US' | 'unknown'` em `agent/infrastructure/tts/providers/elevenlabs/utils/` (case-insensitive; trata `pt`, `pt-br`, `pt_BR`, `en`, `en-us`, `en_US`, undefined, string lixo). Testes cobrindo cada branch.
  - `ListAvailableVoicesUseCase.execute({ userId })` — passa a receber `userId`, busca profile via `AssistantProfileRepository` (já injetado no `PersonalityModule`, expor se preciso), resolve `outputLanguage` (se MNT-217 landar `auto`, resolve pra `pt_BR|en_US` aqui também; senão usa direto). Marca `languageMatch` por voz. Cache atual (5min in-memory) vira `Map<outputLanguage, CacheEntry>` — evita mistura entre users com preferências diferentes.
  - DTO de response `GET /agent/voices` mantém shape flat `{ voices: TtsVoice[] }`, agora com `languageMatch` no item. Grouping é decisão de render → fica no front (escape hatch "layout math local" da memória).
  - Front `AssistantSettingsVoice`: 2 seções — **"Recommended for your language"** (`match`) e **"Other languages"** (`mismatch` + `unknown`). Card de voz `mismatch` ganha badge âmbar `"May sound off in <language>"`; `unknown` fica sem badge (não afirma nada). Sem bloqueio de clique — user escolhe qualquer voz, é nudge visual. `MAX_VISIBLE_PER_SECTION=12`. **Nota**: se o pool ficar desbalanceado (muitas EN, poucas PT), a solução operacional é MNT-251 (adicionar vozes PT à library da conta ElevenLabs), não mudar a regra de bucketing.
  - `AssistantProfileService` (`web/src/services/`) — `TtsVoice` interface no client passa a incluir `languageMatch` (property-direct do backend, sem reshape).
  - Testes: (a) `mapElevenLabsLanguage` cobre `pt`, `pt-br`, `PT_BR`, `en`, `en-us`, `fr` (→ `unknown`), undefined (→ `unknown`), `""` (→ `unknown`); (b) `resolveLanguageMatch` puro (match/mismatch/unknown por combinação de voice lang × outputLanguage); (c) `ListAvailableVoicesUseCase` marca `languageMatch` correto por `outputLanguage`; (d) cache separa por outputLanguage; (e) endpoint retorna shape novo com auth (supertest); (f) `AssistantSettingsVoice.spec.tsx` renderiza 2 seções e badge só nas mismatch.
  - **NÃO inclui**: recompor `outputLanguage` pra ter `auto` (bug separado do MNT-217), filtro por gênero/idade/accent, mudança no playbook de `preview_voice`/`set_voice` (nudge do LLM já existe).

- [ ] **MNT-251** 🛑 [HUMANO][OPS] Enriquecer voice library com vozes PT-BR no dashboard ElevenLabs — hoje `GET /v1/voices` (usado por MNT-56/MNT-250) devolve só as vozes da conta, que hoje pende quase 100% pro inglês (só 2 vozes marcadas `labels.language: 'pt'`). Ação: entrar em https://elevenlabs.io → **Voices** → **Voice Library** → filtro `Language: Portuguese` → escolher ~8-12 vozes variadas (gênero/idade/tom) → clicar **Add to My Voices** em cada. Depois disso o próximo fetch do `ListAvailableVoicesUseCase` (cache de 5min invalida sozinho) já pega elas, e a seção "Recommended for your language" no `AssistantSettingsVoice` fica balanceada em PT e EN. Zero mudança de código. Se depois precisar automatizar isso pra outros users da app sem depender de dashboard, virar task de integração com `POST /v1/voices/add/{public_user_id}/{voice_id}` no backend (fora do escopo agora).
