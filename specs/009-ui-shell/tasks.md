...
- [ ] **MNT-244** [T][S] **Backend — side effect `chartOpen`** (Bundle 4 do 006-visualizations). Adicionar kind `chartOpen` em `api/src/agent/tools/domain/types/tool-side-effect.ts` com payload `{ spec, data, meta }`. Novo `AgentSocketEvent.ChartOpen = 'chart.open'` em `api/src/agent/infrastructure/gateways/constants/agent-socket-events.ts`. Handler novo em `SIDE_EFFECT_EMITTERS` (`api/src/agent/infrastructure/gateways/utils/wire-tool-dispatcher.ts`) — mesmo padrão do `redirect`. `CreateVisualizationTool.execute()` retorna `sideEffects: [{ kind: 'chartOpen', spec, data, meta }]` no result. Testes: unit da tool valida sideEffect emitido; teste do wire mocka `sendClientEvent` e valida payload.
