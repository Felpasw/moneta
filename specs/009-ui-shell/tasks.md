...
- [ ] **MNT-100** [T][S] Dashboard `/` (aba **Início**):
  ...
  - [x] **Charts + full DashboardView com dados reais** ✅ (backend MNT-218 fechado + web consumindo). Novo `dashboardService.getView()` chama `GET /dashboard/view`; `dashboardHooks.use()` (classe + `use()` pattern, `useSuspenseQuery` na key `["dashboard","view"]`). `DashboardScreen` refactor: consome um hook só, empty state via `summary.checkingCount === 0` (backend enriched com `checkingCount` no `DashboardSummary` no mesmo commit), KPIs `totalBalance`/`monthIncome`/`monthExpense`/`monthNet` direto do summary, 3 novos `<ChartCard>` renderizando `<BalanceLineChart data={balanceChart}>` (last 30 days), `<MonthlyFlowChart data={monthlyFlow}>` (last 6 months), `<TopCategoriesChart data={topCategories}>` (this month). Órfãos do `mocks/finance.ts` removidos (arquivo deletado). Types migraram pra `services/interfaces/dashboard.interface.ts` (mirror do port backend). **Refactor dos organism charts** pra consumir shapes brutos: `MonthlyFlowChart` recebe `DashboardMonthlyFlowRow[]` e computa `incomePct`/`expensePct` inline via `max` local + `Intl.DateTimeFormat` pro label do mês; `BalanceLineChart` recebe `DashboardBalancePoint[]` e computa `linePath`/`areaPath`/`lastPoint`/`min`/`max` inline via `useMemo(computeGeometry)` (SVG path é formatação pura — escape hatch da regra "no reshape client"); `TopCategoriesChart` recebe `DashboardTopCategory[]` com fallback pra `icon: null`. Testes web: `dashboard.service.spec.ts` (GET /dashboard/view), `useDashboard.spec.tsx` (Suspense + query key), `DashboardScreen.spec.tsx` reescrito (empty via checkingCount=0, saudação, KPIs, charts renderizados). Suite web: 280 passed / 5 baseline pré-existente. Suite api: 781 passed / 15 baseline agent. Lint zerado no escopo
...
- [x] **MNT-243** [T][S] ✅ commit `178f050` Visual polish nas actions do agent + cards de banco:
...
- [~] **MNT-72** [S] **SUPERSEDED por MNT-249** — decisão de UI de chart migrou de shadcn/Recharts pra visx+motion. Ver `specs/006-visualizations/tasks.md` linha 7 (decisão revisada) e MNT-249 abaixo
...
- [ ] **MNT-141** [T][S] Página `/transactions` (MNT-102) — lista virtualizada + filtros + FAB. Row de transaction em cartão mostra badge "Fatura {mês}" pequeno. Click em row abre `<TransactionDetail>` sheet
  - [x] **Filtros por período, tipo e banco** ✅ commit `c077fed` — 3 molecules novos (`BankFilter`+`BankAvatar`+`BankDropdown`+`BankFilterAddButton`; `DateRangeFilter`+`DateRangeTrigger`+`DateRangePopover`; `TransactionTypeFilter`) com data-attributes no lugar de ternários. URL sync via novo hook classe `urlParamsHooks.use()` (`setParam`/`setListParam`/`setParams`). `buildTransactionFilters` util pura + spec. `useTransactions.use(filters?)` sensível ao filter na query key. Fix crítico: `paramsSerializer: { indexes: null }` no axios (Nest+Zod não parseia bracket `types[]`, precisa `types=x&types=y`). Fix DateRange controlled bug (draft local isolado). Layout invertido no row (BankIcon protagonista + badge de direção verde/rose). Summary sem card wrapper. `date-fns` pra presets (elimina helpers puros). Backend MNT-129 já pronto (accountIds/dateFrom/dateTo/types)
  - [ ] Lista virtualizada, FAB `<AddTransactionSheet>`, sheet `<TransactionDetail>`, sub-rota `/transactions/:id`, badge de fatura, filtro por categoria, search debounced — pendentes pra próximos ciclos
...
- [x] **MNT-76** [T][S] ✅ commit `f7c7336` — `<DynamicChart>` implementado com **SVG puro + Motion** (segue padrão do projeto, sem Recharts — reusa o approach dos `BalanceLineChart`/`MonthlyFlowChart`/`TopCategoriesChart` existentes). 3 widgets organism: `DynamicBarChart` (cobre `bar`+`stacked-bar` degradado), `DynamicLineChart` (cobre `line`+`area` via `variant`), `DynamicPieChart` (cobre `pie`+`donut` via `variant`, slices monocromáticos `fill-white` com opacidade decrescente pra combinar com o padrão da app). Types mirror do backend em `web/src/services/interfaces/chart.interface.ts` (enums pra `ChartType`/`XField`/`Grouping`/`YField`/`Aggregation`/`DatePreset`/`RollingUnit`/`AggregatedFrom`, `TransactionType` reusado do `transactions.interface.ts`). `DynamicChart` faz dispatch via `Record<ChartType, Renderer>` exhaustive. `scatter` e `heatmap` fallback com `role="status"` "not supported yet". MNT-72 (`shadcn add chart` + Recharts) SKIPPED — evita dep, mantém SVG idiomático do projeto.
- [ ] **MNT-77** [DEFERRED] Integração inline no `<MessageBubble>` do chat — renderizar `<DynamicChart>` dentro do bubble quando `toolResults[i].name === 'create_visualization'`. **Adiado até text-mode existir** — em voice-mode (único modo atual), o gráfico entra via takeover fullscreen (MNT-244..246), não inline. Quando o text-mode chegar, esta task volta pra atender leitura em thread persistente.
- [x] **MNT-244** [T][S] ✅ commit `d01699e` — **Backend — side effect `chartOpen`**. `ToolSideEffect` virou union discriminada (`redirect | chartOpen`). `AgentSocketEvent.ChartOpen = 'chart.open'` adicionado. `SIDE_EFFECT_EMITTERS` refatorado pra `Record<kind, emitter>` com narrow interno via `if kind !== ...` (evita cast + mantém lint clean). `CreateVisualizationTool.execute()` retorna `sideEffects: [{ kind: 'chartOpen', spec, data, meta }]`. Frontend agora pode escutar `chart.open` após `tool.result` (MNT-245/246).
- [x] **MNT-245** [T][S] ✅ commit `17d2a97` — **`<ChartTakeoverOverlay>`** organism em `web/src/components/organisms/`. Zustand store `chartTakeoverStore` seguindo o pattern actions-object do projeto (`agentSessionActions`). Overlay `fixed inset-0 z-50` renderiza `<DynamicChart>` + botão voltar (ArrowLeft) + título do spec. ESC handler via useEffect. `role="dialog"` + `aria-modal`. `AnimatePresence` fade. Montado no root `layout.tsx`. Hardware back Capacitor DEFERRED (falta `@capacitor/app` dep).
- [x] **MNT-246** [T][S] ✅ commit `50f577b` — **Socket wiring `chart.open`**. `AgentSocketEvent.ChartOpen` adicionado (mirror do backend). `makeChartDispatcher` no padrão do `makeSystemDispatcher` — string guard, JSON.parse com try/catch, type check. `ChartEnvelope { type, spec, data }` (spec/data required — backend sempre manda). Wire no `useAgentSession`: `dispatchChart` roda em cada `ws.onmessage`, roteia `chart.open` → `chartTakeoverActions.open`. Fluxo end-to-end fechado: voz → LLM → tool → sideEffect → socket → overlay.
- [~] **MNT-247** [S] **REJECTED** — Investigação: `react-day-picker@10.0.1` (usado no `<DateRangeFilter>` do MNT-141) tem `date-fns@4.4.0` como dep direta. `date-fns` fica no bundle mesmo se removido do `package.json`. Adicionar `dayjs` no `/web` sem tirar `date-fns` = 2 libs no bundle pro mesmo caso de uso. Aceita-se a inconsistência conceitual back/front como consequência da dep transitiva. Divisão registrada: back = `dayjs` (manipulação), front = `date-fns` (`web/src/utils/dateRange.ts` only) + `Intl.DateTimeFormat` (formatação display). Reabrir só se `react-day-picker` for substituído.
- [x] **MNT-91** [T][S] ✅ commit `42e03dc` — Página `/charts` implementada full-stack. Backend: `SavedChartsController` expõe REST endpoints (`GET /saved-charts`, `POST /saved-charts`, `POST /:id/run`, `PATCH /:id/name`, `PATCH /:id/pin`, `DELETE /:id`) scopeados por `JwtAuthGuard` + `CurrentUser`, defense-in-depth zod parse no `/run` (spec corrompido → 404). Frontend: rota `/charts` com grid de `<SavedChartCard>` (pin/rename/delete/open), click chama mutation `run` e reusa `chartTakeoverActions.open` pra abrir o `<ChartTakeoverOverlay>` existente (MNT-245) — zero duplicação. Service class + hook class-com-`use()` no padrão `dashboardService`/`useDashboard`. "Open in chat" e thumb DEFERRED (não bloqueante). Rename/delete usam `window.prompt`/`confirm` nativo — funcional mas feio, débito registrado pra trocar por `<Dialog>` shadcn depois.
- [ ] **MNT-249** [T][S] **Migrar `<DynamicChart>` de SVG puro pra visx + motion/react**. Motivação: implementação atual (MNT-76) apresenta artefatos visuais (linhas incompletas em datasets sparse, cálculo de escala manual sem edge case pra range zero em algumas séries). Escopo: 3 organismos (`DynamicBarChart`, `DynamicLineChart`, `DynamicPieChart`) + 3 specs de teste em `web/test/components/organisms/`. Deps adicionados: `@visx/scale`, `@visx/shape`, `@visx/curve` (usar `curveMonotoneX` no line chart pra suavidade). **Preservar**: (1) paleta monocromática via CSS var `--foreground` (bar/line/area) + opacidades escalonadas `[1, 0.82, 0.66, ...]` (pie/donut), (2) pattern de animação com `motion.rect`/`motion.path` (`initial`/`animate`/`transition` com bezier custom `[0.16, 1, 0.3, 1]` + stagger `delay: idx * 0.04`), (3) contrato observável dos testes (`data-testid="bar|line-path|area-fill|slice"`, `data-value` no bar). **Ganhos futuros destrancados**: `layoutId` do motion pra morphing entre chart types no takeover (feature killer no voice-mode); axis/tooltip de `@visx/axis`/`@visx/tooltip` quando forem necessários. `BalanceLineChart`/`MonthlyFlowChart`/`TopCategoriesChart` ficam fora do escopo desta task (SVG manual funcional, migração oportunista futura). Bônus: refactor do `<ChartTakeoverOverlay>` de fullscreen pra **bottom sheet 72vh** com backdrop-blur, drag-to-dismiss, click-out-close, header com botão X (era ArrowLeft). Voice orb (`GlobalAssistant`) sobe pra `z-50` sobre o sheet — user pode continuar falando enquanto vê o chart.
- [ ] **MNT-250** [T][S] **Chart type switcher no bottom sheet — user escolhe formato após render**. Motivação: hoje o agente decide sozinho o `chartType` no `ChartSpec` e o user tem zero controle. Adiciona `<ChartTypeSwitcher>` molecule (tabs Bar/Line/Area/Pie via `role=tablist`) renderizado entre o header e o chart no `<ChartTakeoverOverlay>`. Store ganha `selectedType: ChartType | null` + action `setSelectedType`; overlay passa `spec = { ...spec, chartType: selectedType ?? spec.chartType }` pro `<DynamicChart>`. Zero mudança no backend — client-side puro, mesma data, layout diferente. `selectedType` reseta pra `null` a cada `open` (novo chart → default do agente). Não restringe tipos por compatibilidade (user pode forçar line em categoria — feedback natural). **Deferred**: persistir `preferredType` no `saved_charts.spec` (usuário pinar tipo favorito por saved chart); assistente ler `assistant_preferences.preferredChartType` global.
