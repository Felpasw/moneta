...
- [~] **MNT-76** [T][S] `<DynamicChart>` implementado com **SVG puro + Motion** (segue padrão do projeto, sem Recharts — reusa o approach dos `BalanceLineChart`/`MonthlyFlowChart`/`TopCategoriesChart` existentes). 3 widgets organism: `DynamicBarChart` (cobre `bar`+`stacked-bar` degradado), `DynamicLineChart` (cobre `line`+`area` via `variant`), `DynamicPieChart` (cobre `pie`+`donut` via `variant`). Types mirror do backend em `web/src/services/interfaces/chart.interface.ts`. `DynamicChart` faz dispatch via `Record<ChartType, Renderer>` exhaustive. `scatter` e `heatmap` fallback com `role="status"` "not supported yet". MNT-72 (`shadcn add chart` + Recharts) SKIPPED — evita dep, mantém SVG idiomático do projeto.
...
- [ ] **MNT-245** [T][S] **Frontend — `<ChartTakeoverOverlay>`** (organism em `web/src/components/organisms/`). Overlay fullscreen (não route) que renderiza `<DynamicChart spec data />` (MNT-76) + `<BackButton>` (top-left) + título do spec. Zustand slice `chartTakeover: { open: boolean; spec?; data?; meta? }` com `open()` / `close()`. Fecha via botão voltar, tecla ESC, e back button OS (Capacitor `App.addListener('backButton')`). z-index acima do chat, `<motion>` fadeIn/scale. Chat state atrás fica intocado — ao fechar, user volta ao ponto exato da conversa. Testes: overlay abre com dados corretos, ESC fecha, back-button chama `close()`.
- [ ] **MNT-246** [T][S] **Frontend — socket wiring `chart.open`**. Adicionar `ChartOpen = 'chart.open'` em `web/src/hooks/constants/useAgentSession.constants.ts` (mirror do backend). Novo dispatcher `makeChartDispatcher({ onOpenChart })` em `useAgentSession.utils.ts` (irmão do `makeSystemDispatcher`). Em `useAgentSession.ts`, ao receber envelope `chart.open` → chama slice `chartTakeover.open({ spec, data, meta })`. TTS toca em paralelo naturalmente (canal `tts.audio.*` já independente). Teste: envelope válido dispara `onOpenChart` com payload correto.
