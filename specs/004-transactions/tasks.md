...
- [x] **MNT-132** [T][S] `TransferRepository` + use-cases `CreateTransfer({ fromAccountId, toAccountId, amount, description?, occurredAt })`, `ListTransfers(filters)`, `DeleteTransfer({ id })`. Validações: `fromAccountId !== toAccountId`, ambas as contas pertencem ao user, `amount > 0` ✅ commit `<pendente>` (from=to rejeitado no use-case via SameAccountTransferError → 400; ownership via updateMany.count no repository → AccountNotFoundError → 404; amount>0 via zod DTO → 400; list aceita accountIds array e filtra transfers em que a conta aparece como from OU to)
- [x] **MNT-133** [T][S] Ajuste atômico de saldo em ambas contas na mesma tx DB. Delete reverte. Testes: from=to rejeitado; contas de outro user rejeitadas; saldo consistente após operações mistas ✅ commit `<pendente>` (create debita from e credita to no mesmo `$transaction` do Prisma; delete inverte a operação; testes mockam `$transaction` cobrindo os 3 caminhos + rejeição por ownership em qualquer das duas contas; rollback real em erro mid-tx fica pra bundle Testcontainers)
- [x] **MNT-134** [T][S] Tools: `create_transfer`, `list_transfers`, `delete_transfer`. Playbook do create enfatiza: confirmar 3 campos (from, to, amount); avisar que não conta como gasto no dashboard ✅ commit `<pendente>` (3 tools registradas via @RegisterAssistantTool em `transfers/tools/`; sub-módulo `transfers-tools.module.ts` dedicado no padrão dos outros domínios; validação via zod dentro do execute; erros de domínio (SameAccountTransferError, AccountNotFoundError, TransferNotFoundError) viram ok:false)
