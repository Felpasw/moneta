...
- [x] **MNT-131** [T][S] Tools (bundle) — cada com playbook detalhada: ✅ commit `<pendente>` (6 tools registradas em `transactions/tools/`; alem do CRUD single, tem **add_transactions** e **edit_transactions** batch — ambos rodam N operações num único `$transaction` do Prisma, atomicamente; max 50 itens por batch)
