...
- ~~Reusar o mesmo WS que já roda ou criar namespace separado (`/agent/state`)?~~ → **reusa** o `/agent/ws` existente (multiplex por `type`), definido em MNT-228 back.
- ~~`humanLabel` gerado no back (mais controle) ou derivado no front por `toolName` (menos payload, mais i18n)?~~ → **híbrido**: back manda `caption: { key, params }` estruturado; front traduz via i18n. Novo idioma = 1 arquivo de locale, zero linha no back.
