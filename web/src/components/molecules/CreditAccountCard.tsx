...
export function CreditAccountCard({ row }: CreditAccountCardProps) {
  return (
    <motion.article
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="rounded-xl bg-white/10 p-2">
              <BankIcon bankName={row.bankName} size={28} />
            </div>
          ...
          {row.invoiceStatus !== null ? (
            <span className="shrink-0 rounded-full bg-white/15 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide">
              {INVOICE_STATUS_LABEL[row.invoiceStatus]}
            </span>
          ) : null}
        </div>
      ...
      <div className="mt-auto space-y-4 border-t border-white/10 p-5">
        <div className="space-y-2">
          <div className="flex items-center justify-between text-xs opacity-60">
            ...
            <span>{row.usagePct}%</span>
          </div>
          <div className="h-1.5 overflow-hidden rounded-full bg-white/15">
            <motion.div
              initial={{ width: 0 }}
              animate={{ width: `${row.usagePct}%` }}
        ...
        <dl className="grid grid-cols-3 gap-2 text-xs">
          ...
          <div>
            ...
            <dd className="mt-0.5 text-sm font-medium">
              {row.dueDate !== null ? formatDayMonth(row.dueDate) : "—"}
            </dd>
          </div>
        </dl>
      </div>
