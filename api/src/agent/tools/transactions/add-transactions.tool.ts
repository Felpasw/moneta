...
export class AddTransactionsTool implements AssistantTool {
  ...
  readonly playbook =
    'Registra várias transações de DÉBITO de uma vez, atomicamente — ou TODAS entram ou NENHUMA. Afeta balance das contas, NÃO entra em fatura de cartão. Use quando o user reporta múltiplos gastos no débito/Pix/dinheiro ou várias receitas no mesmo turno (ex: "semana passada gastei 10 no café, 25 no uber, 150 no mercado"). Cada item tem os mesmos campos que a versão single (amount positivo, type diferencia expense/income, occurredAt obrigatório). Confirme com o user TODOS os itens antes — mostre um resumo. Máximo 50 itens por batch. PROIBIDO neste tool: qualquer item que envolva CARTÃO DE CRÉDITO ou PARCELAMENTO — nunca crie múltiplos itens com "Parcela X/N" na descrição simulando parcelamento no débito (isso corrompe balance e não gera fatura). Compras no cartão vão pro tool de credit purchases (batch); compra parcelada vai pro tool de installment purchase. Se qualquer item tiver conta inválida ou dado ruim, o batch inteiro é revertido — nenhum saldo é alterado. Ordem de recebimento é preservada no retorno.';
